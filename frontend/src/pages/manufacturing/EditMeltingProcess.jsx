...
const EditMeltingProcess = () => {
  ...
  const fetchProcessData = async () => {
    try {
      ...
      setFormData({
        melting_date: data.melting_date.split('T')[0],
        heat_no: data.heat_no.toString(),
      ...
      setScrapTotal(parseFloat(data.scrap_total));
      calculateScrapTotal(data.scrap_weight);

      if (data.spectro_readings && data.spectro_readings.length > 0) {
        setSpectroReadings(data.spectro_readings.map((reading, index) => ({
          id: reading.id || Date.now() + index,
          carbon: reading.carbon || '',
          silicon: reading.silicon || '',
          manganese: reading.manganese || '',
          phosphorus: reading.phosphorus || '',
          sulphur: reading.sulphur || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...formData,
        heat_no: parseInt(formData.heat_no),
      ...
      await apiService.updateMeltingProcess(id, payload);
      showToast('Melting process updated successfully', 'success');
      navigate('/manufacturing/melting');
    } catch (error) {
      showToast(error.message || 'Failed to update melting process', 'error');
    } finally {
      setSaving(false);
    }
  };
    ...
    return (
      <div className="flex h-screen items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Loading melting process...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Edit Melting Process</h1>
        <p className="mt-1 text-sm text-gray-500">
          Update melting process details and spectro readings
        </p>
      </div>
...
export default EditMeltingProcess;
